...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-brand-cream">
      ...
      <footer id="contact" className="mt-8 bg-brand-ink text-white"><div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:px-6 md:grid-cols-3"><div><div className="flex items-center gap-3 text-xl font-black"><img src="/assets/logo.jpeg" alt={`${BUSINESS_NAME} logo`} className="h-12 w-12 rounded-full object-cover" /><span>{BUSINESS_NAME}</span></div><p className="mt-3 text-sm leading-6 text-stone-300">Fresh homemade food, made for you and delivered near your university.</p></div><div><h3 className="font-bold">Contact</h3><p className="mt-3 text-sm text-stone-300">WhatsApp: 03234524396</p><a href={`https://wa.me/${BUSINESS_WHATSAPP_NUMBER}`} target="_blank" rel="noreferrer" className="mt-3 inline-block font-bold text-green-300 hover:text-white">Chat on WhatsApp →</a></div><div><h3 className="font-bold">Opening Hours</h3><p className="mt-3 text-sm text-stone-300">Every day: 11:00 AM – 10:00 PM</p><p className="mt-2 text-sm text-stone-300">Location: Near your university</p></div></div><div className="border-t border-white/10 py-5 text-center text-xs text-stone-400">© {new Date().getFullYear()} {BUSINESS_NAME}. Homemade with care.</div></footer>
    </main>
  );
}
