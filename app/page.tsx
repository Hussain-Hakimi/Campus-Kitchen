...
const BUSINESS_NAME = "Halal Biryani House";
...
export default function Home() {
  ...
  function orderOnWhatsApp() {
    ...
    const message = [
      `Hello ${BUSINESS_NAME}! 🍛`, "", "I would like to place an order.", "", "Order:", ...lines, "",
      `Total: ${money(total)}`, "", `Customer Name: ${name.trim()}`, `Phone: ${phone.trim()}`,
      `Delivery Address: ${address.trim()}`, note.trim() ? `Note: ${note.trim()}` : "", "", "Thank you!",
    ].filter(Boolean).join("\n");
    ...
    <main className="min-h-screen bg-brand-cream">
      <header className="sticky top-0 z-50 border-b border-orange-100/80 bg-brand-cream/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
          <a href="#home" className="flex items-center gap-3 font-bold text-brand-ink"><img src="/assets/logo.jpeg" alt={`${BUSINESS_NAME} logo`} className="h-11 w-11 rounded-full object-cover" /><span className="hidden sm:inline">{BUSINESS_NAME}</span></a>
      ...
      <section id="home" className="mx-auto grid max-w-6xl items-center gap-10 px-4 py-12 sm:px-6 md:grid-cols-2 md:py-20">
        ...
        <div className="relative overflow-hidden rounded-[2rem] shadow-soft"><img src="/assets/Chiken-Biryani.jpeg" alt="Fresh homemade chicken biryani" className="h-[360px] w-full object-cover sm:h-[440px]" /><div className="absolute bottom-5 left-5 rounded-2xl bg-white/95 px-4 py-3 shadow-lg"><p className="text-xs font-bold uppercase tracking-wider text-brand-green">Made fresh</p><p className="font-bold">Comfort food, close to campus.</p></div></div>
      </section>

      <section id="menu" className="mx-auto max-w-6xl px-4 py-12 sm:px-6"><div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="font-bold text-brand-orange">OUR MENU</p><h2 className="mt-1 text-3xl font-black sm:text-4xl">Choose your meal</h2></div><div className="flex flex-wrap gap-2">{categories.map((cat) => <button key={cat} onClick={() => setCategory(cat)} className={`rounded-full px-4 py-2 text-sm font-bold ${category === cat ? "bg-brand-green text-white" : "bg-white text-stone-600 ring-1 ring-stone-200 hover:ring-brand-orange"}`}>{cat}</button>)}</div></div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{filteredItems.map((item) => <article key={item.id} className="overflow-hidden rounded-3xl bg-white shadow-soft transition hover:-translate-y-1"><img src={item.image} alt={item.name} className="h-48 w-full object-cover" loading="lazy" onError={(event) => { event.currentTarget.style.display = "none"; }} /><div className="p-5"><div className="flex items-start justify-between gap-3"><h3 className="text-lg font-black">{item.name}</h3><span className="whitespace-nowrap rounded-full bg-orange-50 px-3 py-1 text-sm font-black text-brand-orange">{money(item.price)}</span></div><p className="mt-2 min-h-12 text-sm leading-6 text-stone-500">{item.description}</p><button onClick={() => addToCart(item)} className="mt-4 w-full rounded-2xl bg-brand-orange px-4 py-3 font-bold text-white hover:opacity-90">Add to Order</button></div></article>)}</div></section>
      ...
      <footer id="contact" className="mt-8 bg-brand-ink text-white"><div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:px-6 md:grid-cols-3"><div><div className="flex items-center gap-3 text-xl font-black"><img src="/assets/logo.jpeg" alt={`${BUSINESS_NAME} logo`} className="h-12 w-12 rounded-full object-cover" /><span>{BUSINESS_NAME}</span></div><p className="mt-3 text-sm leading-6 text-stone-300">Fresh homemade food, made for you and delivered near your room.</p></div><div><h3 className="font-bold">Contact</h3><p className="mt-3 text-sm text-stone-300">WhatsApp: 03234524396</p><a href={`https://wa.me/${BUSINESS_WHATSAPP_NUMBER}`} target="_blank" rel="noreferrer" className="mt-3 inline-block font-bold text-green-300 hover:text-white">Chat on WhatsApp →</a></div><div><h3 className="font-bold">Opening Hours</h3><p className="mt-3 text-sm text-stone-300">Every day: 7 PM – 9 PM</p><p className="mt-2 text-sm text-stone-300"></p></div></div><div className="border-t border-white/10 py-5 text-center text-xs text-stone-400">© {new Date().getFullYear()} {BUSINESS_NAME}. Homemade with care.</div></footer>
    </main>
  );
}
