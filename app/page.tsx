  ...
  return (
    <main className="min-h-screen bg-brand-cream">
      <header className="sticky top-0 z-50 border-b border-orange-100/80 bg-brand-cream/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <nav className="hidden items-center gap-7 text-sm font-semibold sm:flex">
            ...
            <a href="#menu" className="hover:text-brand-orange">Menu</a>
      ...
      <section id="home" className="mx-auto grid max-w-6xl items-center gap-10 px-4 py-12 sm:px-6 md:grid-cols-2 md:py-20">
        <div>
          <div className="mb-5 inline-flex rounded-full bg-green-100 px-4 py-2 text-sm font-bold text-brand-green">🏠 Homemade • Fresh • Halal</div>
          <h1 className="text-5xl font-black leading-tight tracking-tight sm:text-6xl">Fresh Homemade Food,<br /><span className="text-brand-orange">Made for You.</span></h1>
          <p className="mt-5 max-w-xl text-lg leading-8 text-stone-600">Delicious homemade meals prepared fresh and delivered near your university.</p>
      ...
      <section id="menu" className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="font-bold text-brand-orange">OUR MENU</p><h2 className="mt-1 text-3xl font-black sm:text-4xl">Choose your meal</h2></div><div className="flex flex-wrap gap-2">{categories.map((cat) => <button key={cat} onClick={() => setCategory(cat)} className={`rounded-full px-4 py-2 text-sm font-bold ${category === cat ? "bg-brand-green text-white" : "bg-white text-stone-600 ring-1 ring-stone-200 hover:ring-brand-orange"}`}>{cat}</button>)}</div></div>
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {filteredItems.map((item) => <article key={item.id} className="overflow-hidden rounded-3xl bg-white shadow-soft transition hover:-translate-y-1">
            <img src={item.image} alt={item.name} className="h-48 w-full object-cover" loading="lazy" />
            <div className="p-5"><div className="flex items-start justify-between gap-3"><h3 className="text-lg font-black">{item.name}</h3><span className="whitespace-nowrap rounded-full bg-orange-50 px-3 py-1 text-sm font-black text-brand-orange">{money(item.price)}</span></div><p className="mt-2 min-h-12 text-sm leading-6 text-stone-500">{item.description}</p><button onClick={() => addToCart(item)} className="mt-4 w-full rounded-2xl bg-brand-orange px-4 py-3 font-bold text-white hover:opacity-90">Add to Order</button></div>
          </article>)}
        </div>
      </section>

      <section id="order" className="mx-auto grid max-w-6xl gap-6 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_1fr]">
        <div className="rounded-3xl bg-white p-6 shadow-soft sm:p-8"><p className="font-bold text-brand-green">YOUR ORDER</p><h2 className="mt-1 text-3xl font-black">Cart & total</h2><div className="mt-6 space-y-3">{cart.length === 0 ? <p className="rounded-2xl bg-brand-cream p-5 text-stone-500">Your cart is empty. Add something delicious from the menu.</p> : cart.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-2xl bg-brand-cream p-3"><div><p className="font-bold">{item.name}</p><p className="text-sm text-stone-500">{money(item.price)} each</p></div><div className="flex items-center gap-2"><button onClick={() => changeQuantity(item.id, -1)} className="h-8 w-8 rounded-full bg-white font-black">−</button><span className="w-5 text-center font-bold">{item.quantity}</span><button onClick={() => changeQuantity(item.id, 1)} className="h-8 w-8 rounded-full bg-white font-black">+</button></div></div>)}</div><div className="mt-6 flex items-center justify-between border-t pt-5 text-xl font-black"><span>Total</span><span className="text-brand-orange">{money(total)}</span></div></div>
